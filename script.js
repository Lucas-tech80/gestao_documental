...
const supabaseUrl = 'https://imdwkxhbcohyyczhvsil.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImltZHdreGhiY29oeXljemh2c2lsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc3NTYyMDQsImV4cCI6MjEwMzMzMjIwNH0.j0Gb8UB5ZmNNZMAudiEvjK8XcZLWLzvPFbDurwtmMZM';

let supabaseClient = null;

// Itens confirmados na planilha "documentação" (mapeamento feito por auditoria).
const PLANILHA_DOCUMENTO_IDS = new Set([
  ...Array.from({ length: 55 }, (_, index) => index + 1),
  57, 58,
  ...Array.from({ length: 14 }, (_, index) => index + 60)
]);
  ...
  Alpine.data('dashboard', () => ({
    ...
    async carregarNotificacoes() {
      ...
      try {
        ...
        this.notificacoes = (data || []).filter((item) => PLANILHA_DOCUMENTO_IDS.has(Number(item.documento_id)));
    ...
    async carregar() {
      ...
      try {
        ...
        const { data, error } = await supabaseClient
          .from('gestao_documental')
          .select('*');

        if (error) throw error;

        this.documentos = (data || [])
          .filter((doc) => PLANILHA_DOCUMENTO_IDS.has(Number(doc.id)))
    ...
    get documentosVisiveisNaListagem() {
      return this.filteredDocumentos
        .filter((doc) => !DOCUMENTOS_OCULTOS_NA_LISTAGEM.has(Number(doc.id)))
        .map((doc) => ({ ...doc, ...(DOCUMENTOS_APRESENTACAO_PLANILHA[Number(doc.id)] || {}) }));
    },

    statusResumoGeral(doc) {
      if (DOCUMENTOS_OCULTOS_NA_LISTAGEM.has(Number(doc && doc.id))) return 'em_dia';
    ...
    async openNotificationDocument(item) {
      ...
      if (!PLANILHA_DOCUMENTO_IDS.has(Number(item.documento_id))) return;
